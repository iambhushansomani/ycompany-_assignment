    ...
    public static class DbInitializer
    {
        public static void Initialize(ApplicationDbContext context)
        {
            ...
            context.SaveChanges();
                             ...
                             // Add documents and invoices
            var documentsPath = Path.Combine(Directory.GetCurrentDirectory(), "Properties", "Documents");
            var invoicesPath = Path.Combine(Directory.GetCurrentDirectory(), "Properties", "Invoices");

            for (int i = 1; i <= 11; i++)
            {
                var documentPath = Path.Combine(documentsPath,"document.pdf");
                var invoicePath = Path.Combine(invoicesPath,"invoice.pdf");
                Console.WriteLine(documentPath);
                Console.WriteLine(File.Exists(documentPath));
                if (File.Exists(documentPath))
                {
                    var document = new Document
                    {
                        ...
                        Content = File.ReadAllBytes(documentPath),
                        ClaimId = claims[i - 1].ClaimId,
                        DocumentType = "Document"
                    };
                    context.Documents.Add(document);
                }

                if (File.Exists(invoicePath) && (claims[i - 1].Status == "Work Completed - Payment Pending" || claims[i - 1].Status == "Payment Completed" || claims[i - 1].Status == "Completed & Delivered"))
                {
                    var invoice = new Document
                    {
                        ...
                        ContentType = "application/pdf",
                        Content = File.ReadAllBytes(invoicePath),
                        ClaimId = claims[i - 1].ClaimId,
                        DocumentType = "Invoice"
                    };
                    context.Documents.Add(invoice);
                    claims[i - 1].InvoiceDocumentUuid = $"invoice-uuid-{i}";
                }
            }
